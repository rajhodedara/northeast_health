...
const Header = ({ userRole = 'asha', isOffline = false, syncStatus = 'synced', onLanguageChange, currentLanguage = 'en' }) => {
  ...
  const ashaNavItems = [
    { path: '/asha-data-collection', label: 'Data Collection', icon: 'FileText' },
                ...
                <div className="absolute right-0 top-full mt-1 w-48 bg-popover border border-border rounded-md shadow-modal z-1100 animate-fade-in">
                  {languages?.map((language) => (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-2 text-sm font-body text-left hover:bg-accent rounded-md transition-colors duration-150 ${
                        currentLanguage === language?.code ? 'bg-accent text-accent-foreground' : 'text-popover-foreground'
                      }`}
                    >
              ...
              <div className="border-t border-border pt-2 mt-2">
                ...
                {languages?.map((language) => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-3 text-base font-body text-left hover:bg-accent rounded-md transition-colors duration-200 ${
                      currentLanguage === language?.code ? 'bg-accent text-accent-foreground' : 'text-text-secondary'
                    }`}
                  >
                    <span>{language?.name}</span>
